...
話さなければならないことは沢山あるが、次の 3 つの点には必ず気を付けるべき。 1) 誰でも再現可能な営業の段取りを作ること 2) 顧客のロールとプロダクト機能を一致させること 3) プロダクト切り替えコストの負担者を明確にすること。データから得られるインサイトは効果が不明瞭だが、そのなかでも「誰でも売れる」明確なユースケースと必然性を感じさせるタイミングが不可欠になる。データを扱う職種はデータアナリストやデータエンジニアなど切り分けられていることが多いため、ターゲットユーザーの職掌を超える機能は評価されても受け入れられない可能性がある。最後に、データは様々な部門や職掌にまたがるため切り替えに伴うステークホルダーを明らかにしないと説得すらできなくなる。詳細 : [シードラウンドで約 8 億円を調達したノーコードのデータ連携ツール Cascade はなぜ歩みを止めたのか](https://note.com/piqcy/n/nb56c1afd460f)

( [Final Thoughts on Cascade](https://www.cascade.io/) より )(2023)
